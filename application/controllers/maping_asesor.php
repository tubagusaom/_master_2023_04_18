...
class Maping_asesor extends MY_Controller {
    ...
    function index() {
        if ($_SERVER['REQUEST_METHOD'] == 'GET') {
            ...
            $grid = $this->grid->set_properties(array('model' => 'maping_asesor_model', 'controller' => 'maping_asesor', 'options' => array('id' => 'maping_asesor', 'pagination', 'rows_number')))->load_model()->set_grid();
    ...
    function sms($id_asesor, $id_jadwal) {
        ...
        // smssend_zenziva($row_asesor->hp, $pesan); //mengantisifasi menghabiskan token sms
        ...
        foreach ($list_notif as $key => $value) {
            // smssend_zenziva($value, $pesan_notif);//mengantisifasi menghabiskan token sms
        }
    }
